      ...
      <nav className="flex-shrink-0 bg-card border-t border-border/30 flex items-center justify-around py-2.5 pb-3">
        {navItems.map((item) => (
          <button key={item.label} onClick={() => { setActiveNav(item.label); setReportProfile(null); setShowFilters(false); }}
            className={`relative flex items-center justify-center w-10 h-10 rounded-xl transition-colors ${activeNav === item.label ? "text-primary bg-primary/10" : "text-muted-foreground hover:text-foreground"}`}>
