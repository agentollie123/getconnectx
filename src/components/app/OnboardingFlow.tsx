...
import { Users, Building2, Rocket, Code, Palette, TrendingUp, Briefcase, Globe, Linkedin, Check, ArrowRight, ArrowLeft, Sparkles } from "lucide-react";
...
export function OnboardingFlow({ onComplete, isPremium }: OnboardingFlowProps) {
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 2: {
        const options = userType === "startup"
          ? ["Co-Founder", "CTO / Technical Lead", "Product Designer", "Growth Marketer", "Operations"]
          : ["Co-Founder", "Join a Team", "Find a Startup", "Explore Ideas"];
        return (
          <motion.div key="s2" variants={slideVariants} initial="enter" animate="center" exit="exit" className="flex flex-col items-center text-center px-6">
            ...
            <div className="w-full max-w-xs space-y-2.5">
              {options.map(opt => (
                <button
                  key={opt}
                  onClick={() => toggleItem(lookingFor, setLookingFor, opt)}
                  className={`w-full px-4 py-3 rounded-xl border-2 text-left text-sm font-medium transition-all flex items-center justify-between ${
                    lookingFor.includes(opt) ? "border-primary bg-primary/10 text-primary" : "border-border bg-card text-foreground hover:border-primary/50"
                  }`}
                >
                  {opt}
                  {lookingFor.includes(opt) && <Check className="w-4 h-4 text-primary" />}
                </button>
              ))}
            </div>
            <Button className="w-full max-w-xs mt-6 bg-primary text-primary-foreground h-11" onClick={next} disabled={lookingFor.length === 0}>
      ...
      case 3: {
        const skillOptions = ["Engineering", "Product", "Design", "Marketing", "Sales", "Operations", "Finance", "Data Science"];
        return (
          <motion.div key="s3" variants={slideVariants} initial="enter" animate="center" exit="exit" className="flex flex-col items-center text-center px-6">
            <h2 className="font-display text-xl font-bold text-foreground mb-2">Tell us about you</h2>
            <p className="text-sm text-muted-foreground mb-6">What skills do you bring?</p>
            <div className="w-full max-w-xs flex flex-wrap gap-2 justify-center">
              {skillOptions.map(skill => {
                const icons: Record<string, any> = { Engineering: Code, Product: Rocket, Design: Palette, Marketing: TrendingUp, Sales: Briefcase, Operations: Globe };
                const Icon = icons[skill] || Sparkles;
                return (
                  <button
                    key={skill}
                    ...
                    className={`px-3 py-2 rounded-xl border text-xs font-medium flex items-center gap-1.5 transition-all ${
                      skills.includes(skill) ? "border-primary bg-primary/10 text-primary" : "border-border bg-card text-foreground hover:border-primary/50"
                    }`}
                  >
            ...
            <Button className="w-full max-w-xs mt-6 bg-primary text-primary-foreground h-11" onClick={next} disabled={skills.length === 0}>
              Continue <ArrowRight className="w-4 h-4 ml-2" />
            </Button>
          </motion.div>
        );
      ...
      case 5:
        return (
          <motion.div key="s5" variants={slideVariants} initial="enter" animate="center" exit="exit" className="flex flex-col items-center text-center px-6">
            ...
            <p className="text-sm text-muted-foreground mb-8 max-w-xs">
              Your profile is ready. Start discovering co-founders, teammates, and startups that match your goals.
            </p>
